...
public class Background_Listener<data_storage_size> {
    ...
    //              [-xxxxxxx] Length of Data (max 127)
    //Second byte:  [xxxxxxxx] What variable is stored
    ...
    private static sending_response send_dat(byte var) {
        ...
        switch (sending_state) {
            case 0:
                ...
                control_byte = (byte) ((control_byte ^ (byte) len) ^ (byte) var);
                for (int x = 0; x < len; x++) {
                    control_byte = (byte) (control_byte ^ send_data_storage[var - 1][x]);
                }
                byte[] data_protocol = {0b00100100, len, var};
    ...
    private static boolean wait_for_requested(byte req) {
        if (((data_storage[0] & (1 << 7)) > 0) && (req == data_storage[1])) {
            return true;
        }
        return false;
    }
                                ...
                                switch (cmd_state) {
                                    ...
                                    case READDATLEN:
                                        if (input >= (1 << 7)) {
                                            ...
                                            data_storage[0] = (byte) (data_storage[0] | input);
                                        }
                                        last_state = cmd_state;
                                    ...
                                    case READDATVAR:
                                        ...
                                        if ((data_storage[0] & (~(1 << 7))) == 0)
                                            cmd_state = commandcontroll.COMP;
                                        break;
                                    case READDAT:
                                        if (data_storage[0] <= data_storage_size)
                                            data_storage[data_storage_counter + 2] = input;
                                        data_storage_counter++;
                                        if (data_storage_counter == (data_storage[0] & (~(1 << 7)))) {
