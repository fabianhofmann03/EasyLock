...
uint8_t ADC_GetConversionResult(void);
...
uint8_t status_storage;


uint8_t hall_codes[] = {127, 103};
        ...
        if (EUSART_is_rx_ready()) {
            ...
            switch (cmd_state) {
                ...
                case READDATLEN:
                    ...
                    break;
                case READDAT:
                    if (data_storage[1] <= 127) data_storage[data_storage_counter + 2] = input;
                        ...
                        switch (cmd_exe_counter) {
                            ...
                            case 2:
                                if(wait_for_requested(PASSWORD)) {
                                    if(compare_pw(&data_storage[2], data_storage[1]) || !PORTBbits.RB0) {
                                        cmd_exe_counter++;
                                    }else {
                                        send_stat(DENY);
                                        stop_cmd();
...
_Bool compare_pw(uint8_t *pw, uint8_t len) {
    _Bool res = 1;
    uint8_t eedata = DATAEE_ReadByte(4);
    if (eedata != len) {
        res = 0;
    } else {
        for (int x = 0; x < len; x++) {
            eedata = DATAEE_ReadByte(x + 4 + 1);
            uint8_t indata = *(pw + x);
            if (eedata != indata) {
                res = 0;
                break;
...
void save_pw(uint8_t *pw, uint8_t len) {
    if (DATAEE_ReadByte(4) != len) DATAEE_WriteByte(4, len);
    for (int x = 0; x < len; x++) if (DATAEE_ReadByte(x + 4 + 1) != *(pw + x)) DATAEE_WriteByte(x + 4 + 1, *(pw + x));
}
...
_Bool door_status_changed(_Bool open_or_close, int16_t val) {
    if(open_or_close) {
        if((hall_codes[0] > hall_codes[1] && val >= hall_codes[1] + ((hall_codes[0] - hall_codes[1]) * 9 / 10)) || (hall_codes[0] < hall_codes[1] && val <= hall_codes[1] - ((hall_codes[1] - hall_codes[0]) * 9 / 10))) return 1;
    }else {
        if((hall_codes[1] > hall_codes[0] && val >= hall_codes[0] + ((hall_codes[1] - hall_codes[0]) * 9 / 10)) || (hall_codes[1] < hall_codes[0] && val <= hall_codes[0] - ((hall_codes[0] - hall_codes[1]) * 9 / 10))) return 1;
    }
    return 0;
}
